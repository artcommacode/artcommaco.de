...
const render = ({title}, html, nextPrev, css) => `
  <!doctype html>
  <html>
    <head>
      <meta charset="utf-8" />
...
const renderNextPrev = (posts, post) => {
  const i = posts.findIndex(({attributes}) => attributes.slug === post.attributes.slug)
  const prevPost = posts[i - 1]
  const nextPost = posts[i + 1]
  const back = '<a class="back" href="/">&darr;</a>'
  const prev = prevPost ? `<a class="prev" href="/posts/${prevPost.attributes.slug}.html"/>&larr;</a>` : ''
  const next = nextPost ? `<a class="next" href="/posts/${nextPost.attributes.slug}.html"/>&rarr;</a>` : ''
  return `<div class="prev-next">${prev} ${back} ${next}</a>`
}

export default (post, css, posts) => {
